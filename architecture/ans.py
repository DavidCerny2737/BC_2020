...
def rel_error(x, y):
    ...
    """
    #print('gradient', x, x.shape)
    #print('num_gradient', y, y.shape)
    help2 = torch.abs(x) + torch.abs(y)
    help = (torch.clamp(help2, min=1e-8))
    return float(torch.max(torch.abs(x - y) / help))
